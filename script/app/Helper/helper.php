...
// amit singh
function load_main_whatsapp()
{
    $option = Option::where('key', 'marketing_tool')->first();
    $option = json_decode($option->value);
    $data = [];
    $data['whatsapp_number'] = $option->whatsapp_number;
    $data['pretext'] = null;
    if (!empty($option->whatsapp_shop_page_pretext)) {
        $data['pretext'] = $option->whatsapp_shop_page_pretext;
    }
    if (!empty($option->whatsapp_other_page_pretext)) {
        $data['pretext'] = $option->whatsapp_other_page_pretext;
    }
    return view('main.whatsapp', compact("data"));
}
...
function amount_format($amount, $type = "icon")
{
    if (url('/') == env('APP_URL')) {
        ...
        if (Cache::has(domain_info('user_id') . 'currency_info')) {
            ...
            // $currency_icon = $value['currency_icon']; amit singh
            $currency_icon = "Rs.";
    ...
    if (Cache::has($url)) {
        return $r = cache()->remember(domain_info('user_id') . 'currency_info', 500, function () {
            ...
            if (!empty($currency)) {
                ...
                // $dta['currency_icon'] = $info->currency_icon;amit singh
                $dta['currency_icon'] = "Rs.";

                return $dta;
            } else {
                $dta['currency_position'] = 'left';
                // $dta['currency_name'] = "USD"; amit singh
                // $dta['currency_icon'] = "$";
                $dta['currency_name'] = "INR";
                $dta['currency_icon'] = "Rs.";
                return $dta;
            }
        });
        ...
        $r = cache()->remember('currency_info', 300, function () {
            ...
            // $dta['currency_icon'] = $currency->currency_icon; amit singh
            $dta['currency_icon'] = "Rs.";
            return $dta;
        });
    }
