...
class Lphelper
{
    ...
    public static function ConfigCategory($type, $select = '', $notid = '')
    {

        if (Auth::user()->role_id == 3) {
            $categories = Category::where('user_id', Auth::id())->whereNull('p_id')->where('name', '!=', 'Uncategorizied')->select('id', 'name', 'p_id')
                ->where('type', $type);
            if (!empty($notid)) {
                $categories = $categories->where('id', '!=', $notid);
            }
