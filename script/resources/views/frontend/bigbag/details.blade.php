                    ...
                    <div class="single-product-info">
                        ...
                        <form method="post" class="cart-form" action="{{ url('/addtocart') }}">
                            ...
                            <div class="single-product-widget product-quantity">
                                ...
                                <ul>
                                    <li><input type="number" name="qty" id="qty" @if ($info->stock->stock_manage == 1)
                                            @if ($info->stock->stock_status==0) disabled
                                    max="0" min="0" @else
                                        max="{{ $info->stock->stock_qty }}" min="1"
                                    value="1" @endif @else min="1" max="999" value="1"
                                        @endif></li>
                                    ...
                                    <li>
                                        <button type="submit" class="btn btn-outline submit_btn" @if ($info->stock->stock_status == 0) disabled @endif>
                                            ...
                                            <span class="submit_text">
                                                ...
                                                {{ __('Out Of Stock') }} @else {{ __('Add to Cart') }}
                                                @endif
                                            </span>
                                        </button>
                                    </li>
                                ...
                                <span class="available_qty @if ($info->stock->stock_manage == 0 ||
                                    $info->stock->stock_status == 0) none @endif">{{ __('Available Quantity') }} : <span id="qty_area">
                                        @if ($info->stock->stock_manage == 1)
                                            {{ $info->stock->stock_qty }} @endif
                                    </span></span>
                            ...
                            @foreach ($info->options as $key => $option)
                                ...
                                <div class="single-product-widget product-tags">
                                    <h5>{{ $option->name }} @if ($option->is_required == 1) <span class="text-danger">*</span>
                                        @endif
                                    </h5>
                                    <ul>
                            ...
                            <div class="single-product-widget product-tags">
                                ...
                                <ul>
                                    @foreach ($info->brands as $row)
                                        <li><a
                                                href="{{ url('/brand/' . $row->slug . '/' . $row->id) }}">{{ $row->name }}</a>
                                        </li>
                                        <input type="hidden" class="cat_id" value="{{ $row->id }}">
                                    @endforeach
                        ...
                        <div class="single-product-widget product-share">
                            ...
                            <ul class="round-icon footer-icon">
                                ...
                                <li><a target="_blank" href="https://twitter.com/intent/tweet?url={{ url()->full() }}"><i
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="section-heading">
                        ...
                        <a href="{{ url('/shop') }}" class="btn btn-outline"><i class="fas fa-eye"></i>
                            {{ __('view all') }}</a>
                    </div>
                </div>
            </div>
