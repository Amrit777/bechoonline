                ...
                <div class="col-12 col-md-12 col-lg-5">
                    <div class="card profile-widget">
                        <div class="profile-widget-header">
                            <img alt="" src="{{ asset('uploads/' . $info->id . '/logo.png') }}"
                                class="profile-widget-picture" height="80">
                            <div class="profile-widget-items">

                                <div class="profile-widget-item">
                                    <div class="profile-widget-item-label">{{ __('Plan') }}</div>
                                    <div class="profile-widget-item-value">{{ $info->user_plan->plan_info->name ?? '' }}
                                    </div>
                                </div>

                                <div class="profile-widget-item">
                                    <div class="profile-widget-item-label">{{ __('Will Expired') }}</div>
                                    <div class="profile-widget-item-value">{{ $info->user_plan->will_expired ?? '' }}
                                    </div>
                                </div>
                                ...
                                <div class="profile-widget-item">
                                    ...
                                    <div class="profile-widget-item-value">
                                        @if ($info->status == 1) <span
                                                class="badge badge-success">{{ __('Active') }}</span>
                                        @elseif($info->status==0) <span
                                                class="badge badge-danger">{{ __('Trash') }}</span>
                                        @elseif($info->status==2) <span
                                                class="badge badge-warning">{{ __('Suspended') }}</span>
                                        @elseif($info->status==3) <span
                        ...
                        <div class="profile-widget-description">
                            <div class="profile-widget-name">{{ $info->name }} <div
                                    class="text-muted d-inline font-weight-normal">
                                    <div class="slash"></div><a target="_blank"
                                        href="{{ $info->user_domain->full_domain ?? '' }}">{{ $info->user_domain->domain ?? '' }}</a>
                                </div>
                            </div>
                            <ul class="list-group">
                                ...
                                <li class="list-group-item">{{ __('Domain :') }}
                                    {{ $info->user_domain->domain ?? '' }}
                                    @if (!empty($info->user_domain))
                                        @if ($info->user_domain->status === 1)
                                            <span class="badge badge-success">{{ __('Active') }}</span>
                                        @elseif($info->user_domain->status === 0) <span
                                                class="badge badge-danger">{{ __('Trash') }}</span>
                                        @elseif($info->user_domain->status === 2) <span
                                                class="badge badge-warning">{{ __('Draft') }}
                                            @elseif($info->user_domain->status === 3) <span
                                                    class="badge badge-warning">{{ __('Pending') }}</span>
                                        @endif
                                    @endif
                                </li>
                                ...
                                <li class="list-group-item">{{ __('Total Orders :') }}
                                    {{ number_format($info->orders_count) }}</li>
                                <li class="list-group-item">{{ __('Total Posts:') }}
                                    {{ number_format($info->term_count) }}</li>
                                <li class="list-group-item">{{ __('Storage Used:') }}
                                    {{ folderSize('uploads/' . $info->id) }}MB /
                                    {{ $info->user_plan->plan_info->storage ?? 0 }} MB</li>

                                <li class="list-group-item">{{ __('Joining Date:') }}
                                    {{ $info->created_at->format('d-F-Y') }}</li>
                            </ul>
                                    ...
                                    <tbody>
                                        @foreach ($histories as $row)
                                            @isset($row->payment_method->trasection_id)
                                                <tr>
                                                    <td><a
                                                            href="{{ route('admin.order.show', $row->id) }}">{{ $row->order_no }}</a>
                                                    </td>
                                                    ...
                                                    <td><a href="{{ route('admin.order.edit', $row->id) }}"
